...
## 8. Webフロントを公開する（AWS Amplify Hosting・S3からの手動デプロイ）

[frontend/index.html](../frontend/index.html)はプレーンHTML+JS（ビルド不要）のチャット画面。API呼び出し先が`__API_ENDPOINT__`というプレースホルダーのままファイルに埋め込まれているため、**手順7-5で控えた呼び出しURLに書き換えてからS3にアップロードし、Amplifyの手動デプロイでそのS3の場所を指定してデプロイする**のがこの手順の流れになる。Gitリポジトリとは連携しない（Amplifyコンソールの「Deploy without Git」フロー）。

### 8-1. `index.html`のAPI URLを書き換える
...
### 8-2. 書き換えた`index.html`をS3にアップロードする

ドキュメント用バケット（手順1）とは別に、フロントのデプロイ元専用のバケットを用意する。zip化は不要で、`index.html`をそのままアップロードする。

1. S3コンソールで**「バケットを作成」**
2. バケット名を入力（例: `<project>-frontend-deploy`）。リージョンは他の手順と同じものを選択
3. 「このバケットのブロックパブリックアクセス設定」は**デフォルトのまま（すべてブロック）**にする（Amplifyはバケットポリシー経由で読み取るため、バケットを公開する必要はない）
4. 「バケットを作成」
5. 作成したバケットを開き、**「フォルダを作成」**で`frontend`という名前のフォルダ（プレフィックス）を作成する
6. `frontend`フォルダを開き、**「アップロード」→「ファイルを追加」**で手順8-1で書き換えた`index.html`を選択し、**「アップロード」**を実行する

### 8-3. Amplifyアプリを作成し、手動デプロイする

1. マネジメントコンソールで**AWS Amplify**を開く
2. 右上の**「新しいアプリの作成」**をクリック
3. 「Amplifyを使ってみる」ページで**「Gitなしでデプロイ」**を選択→**「次へ」**
4. 「手動デプロイを開始」ページで以下を入力する
   - **アプリ名**: 任意（例: `<project>-frontend`）
   - **ブランチ名**: 任意（例: `main`）
   - **方式**: **「Amazon S3」**を選択
5. 「ホストするオブジェクトのS3の場所」で**「参照」**をクリックし、手順8-2で作成したバケットを選択→**「プレフィックスを選択」**で`frontend`フォルダを選択する
6. **「保存してデプロイ」**をクリックする
7. デプロイが完了すると、アプリの詳細画面に表示される**ドメイン**（例: `https://main.xxxxxxxxxx.amplifyapp.com`）がWebフロントのURL。クリックしてブラウザで開く

これで、参加者向けREADMEと同じ「ブラウザからチャットで質問する」体験がマネジメントコンソール操作のみで完成する。

### 8-4. `index.html`を更新した場合の再デプロイ

Gitリポジトリと連携していないため、S3の`index.html`を上書きしただけではAmplify側に反映されない。手順8-2でファイルを再アップロードした後、Amplifyアプリの詳細画面で対象ブランチを開き、**「アクション」→「新しいデプロイの実行」**（または「S3の場所」を選び直して保存）を実行して再デプロイする。
...
| 画面は表示されるが質問すると必ずエラーになる | 手順8-1で`__API_ENDPOINT__`を実際の呼び出しURLに書き換え忘れていないか確認する（プレースホルダーのままだとfetch先が存在せず失敗する） |
| Amplifyのデプロイが「Access Denied」等の権限エラーで失敗する | 手順8-3でS3バケットのブロックパブリックアクセス設定を変更していないか（デフォルトのままでよい）、手順8-2で作成したバケット・プレフィックスをAmplify側で正しく選択できているか確認する。コンソールのウィザード経由なら読み取り用のバケットポリシーは自動生成される |
| ブラウザで開くと更新した内容が反映されない | S3の`index.html`を上書きしただけではAmplifyには反映されない。手順8-4の再デプロイを実行したか確認する。それでも反映されない場合はブラウザキャッシュが原因のことが多く、スーパーリロード（Ctrl+Shift+R等）を試す |
...
1. Amplifyアプリ
2. フロントデプロイ用S3バケット（オブジェクトを空にしてから削除）
3. API Gateway（API本体を削除）
4. Lambda関数
5. LambdaのIAMロール（ロールを削除すればアタッチした管理ポリシーの関連付けも解除される）
6. Bedrock Knowledge Base（データソースごと削除される）
7. Knowledge BaseのIAMロール
8. S3 Vectorsのインデックス→ベクトルバケット
9. S3バケット（ドキュメント。オブジェクトを空にしてから削除）
